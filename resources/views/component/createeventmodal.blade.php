<div id="createEventModal" class="@if($errors->any()) block @else hidden @endif fixed inset-0 z-50">
    ...
    <div class="absolute inset-0 bg-black/40 cursor-pointer backdrop-blur-[1px]"
    ...
    <div class="relative mx-auto my-12 w-[92%] max-w-xl bg-white rounded-xl shadow-lg p-6 overflow-y-auto max-h-[90vh]">
        ...
        <div class="flex items-start justify-between">
            ...
            <span onclick="document.getElementById('createEventModal').classList.add('hidden')"
        ...
        <!-- EVENT FIELDS FORM -->
        <form id="createEventForm" class="mt-5 space-y-4" method="POST" action="{{ route('events.store') }}" enctype="multipart/form-data" novalidate>
            @csrf
            ...
            <div>
                ...
                <input name="name" value="{{ old('name') }}"
                    class="shadow-input mt-1 w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none" required>
                @error('name')
            ...
            <div>
                ...
                <textarea name="description" rows="3"
                    class="shadow-input mt-1 w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none resize-none" required>{{ old('description') }}</textarea>
                @error('description')
                <p class="text-xs text-red-600 mt-1">{{ $message }}</p>
                @enderror
            </div>
...
<!-- <script>
    document.addEventListener('DOMContentLoaded', () => {
        ...
        // Show date/time picker for modern browsers
        ...
        const requiredFields = [
            ...
            form.querySelector('input[name="event_date"]'),
            form.querySelector('input[name="start_time"]'),
            form.querySelector('input[name="end_time"]'),
